...
import torch
from models.bert_attention_model_indic import AttentionModel
from models.bert_cnn_model_indic import BERTCNNSentiment
...
# BERT model title 
# Change here only
bert_model_title = "ai4bharat/indic-bert"
...
models = [  BERTCNNSentiment(bert, OUTPUT_DIM, DROPOUT, N_FILTERS, FILTER_SIZES),
            AttentionModel(bert, BATCH_SIZE, OUTPUT_DIM, HIDDEN_DIM, 50000)  ]
...
optimizers = [optim.Adam(models[0].parameters(),lr), optim.Adam(models[1].parameters(),lr)]
...
def categorical_accuracy(preds, y):
    ...
    for j,i in enumerate(y.cpu().numpy()):
      ...
      if i==0:
        ...
        total2+=1
    metric=torch.FloatTensor([count0/true_correct[0],count1/true_correct[1],count2/true_correct[2],f1_score(y.cpu().detach().numpy(),predictions.cpu().detach().numpy(),average='macro')])
...
test_loss, test_acc, tot = ensemble_evaluate(models, test_iterator, criterion)
